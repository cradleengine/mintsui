...
def init_anime_regen(client=None):
    @client.component("anime_regen")
    async def _anime_regen(ctx: ComponentContext):
        await ctx.defer(ephemeral=True)

        total_message = ctx.message.content
        prompt_group = re.search('Your Prompt: (.*)\n', total_message)
        user_prompt = prompt_group.group(1) + ", in the style of Makoto Shinkai and Takehiko Inoue, Visually stunning, 4K, 8K"
